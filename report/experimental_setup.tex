...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    $c_{\mathrm{travel}}$ & $10^{-5}$--$6\times10^{-5}$ & travel cost per meter \\
    \midrule
  ...
  \caption{Parameters varied across experiments, with the range each is explored
    ...
    intervals shown; the horizontal-comb experiment sweeps a grid of food-site
    count $\{1,2,3,4,6,8,12,16,24\}$ against median patch radius
    $\{15, 37.5, 75, 150, 300, 600\}$ m; and the evolutionary-interaction grid
    uses the discrete values $B \in \{0.30, 0.44, 0.56\}$,
...
running $1024$ trials in total across sixteen parallel workers evaluated against a
...
the three evolutionary parameters, re-centred on the disk stable region,
$B \in \{0.30, 0.44, 0.56\}$, $\sigma_m \in \{0.05, 0.07, 0.09, 0.11\}$, and
$\rho \in \{0.0, 0.3, 0.6, 0.9\}$, evaluating each of the $48$ cells over the same
