...
$0.18$, and read with interpretation noise $0.12$ (\Cref{sec:signals}). The
maximum search distance is $D = 8$ and food sites lie at distances in
$[1, d_{\max}]$. Foraging carries a per-distance travel cost, a dance cost
...
the search limit from $\mathcal{U}(0.15D, 0.45D)$, and both transposition traits
start at $0$, so early foragers signal weakly, rarely attend to one another, and
use only direct pointing.
...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    $D$ & $8$ & maximum search distance \\
    $d_{\min}$ & $1$ & minimum food distance \\
  ...
  \caption{Parameters held fixed across all experiments. Ecological parameters
    (food-site count, angular width, capacity, maximum distance, travel cost) and
...
\Cref{tab:varied-params}: five ecological parameters (food-site count, angular
width, capacity, maximum food distance, and travel cost) and three evolutionary
...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    food-site count       & $1$--$10$       & number of food sites per episode \\
    angular width         & $0.08$--$0.50$  & angular width of each food site \\
    capacity              & $4$--$16$       & foragers a site supplies before exhaustion \\
    $d_{\max}$            & $4.5$--$9.0$    & maximum food-site distance \\
    $c_{\mathrm{travel}}$ & $0.01$--$0.06$  & travel cost per unit distance \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{Parameters varied across experiments, with the range each is explored
    over. Ranges give the union across sub-experiments: the ecological parameters
    and $B,\sigma_m,\rho$ are sampled continuously in the Optuna search over the
    intervals shown; the horizontal-comb experiments sweep food-site count over
    $\{1,2,3,4,6,8\}$ and angular width over $\{0.08,0.15,0.20,0.30,0.50\}$; and
    the evolutionary-interaction grid uses the discrete values
    $B \in \{0.10, 0.25, 0.44\}$, $\sigma_m \in \{0.045, 0.075, 0.090, 0.135\}$,
    and $\rho \in \{0.0, 0.3, 0.6, 0.9\}$. Outside the search, capacity is $6$,
    $d_{\max}$ equals $D = 8$, and the horizontal runs use $\sigma_m = 0.07$.}
...
bias. We refer to the flatten pipeline as v2 and the unproject pipeline as v3.
The two pipelines are identical in every other respect, so comparing them isolates
the effect of the geometric decode assumption on whether and where the transition
occurs.
...
seeds. Conditions comprise three anchors --- a \emph{hard} ecology ($1$ site,
angular width $0.08$), a \emph{baseline} ($2$ sites, width $0.20$), and an
\emph{easy} ecology ($8$ sites, width $0.50$) --- together with two orthogonal
one-dimensional sweeps that share the baseline point: a sweep over food-site
count at fixed width, and a sweep over angular width at fixed site count.

The primary outcome is the \emph{recruitment advantage} (\Cref{sec:foraging}):
among attempts where a dance was available, the success rate of workers that
followed a dance minus that of contemporaneous workers that searched at random.
Because the follow decision is a randomized coin flip on receiver attention within
the same episodes, this is a near-randomized, contemporaneous estimate of what the
...
robustness, in the sequence of sub-experiments below. Each stage draws from a
distinct block of seeds so that tuning and evaluation never overlap.

\paragraph{Parameter search.}
We search nine parameters --- food-site count, angular width, capacity, maximum
distance, and travel cost, together with $B$, $\sigma_m$, and $\rho$ (food value
fixed at $v = 1$) --- with the Tree-structured Parzen Estimator sampler of Optuna,
running $512$ trials with $64$ initial random trials. Each trial evaluates a
panel of $10$ seeds ($100$--$109$) and is scored by
\begin{equation}
...
holding the others at their validated values, over $100$ seeds ($300$--$399$),
to identify which parameters the transition is most sensitive to.
...
the three evolutionary parameters, $B \in \{0.10, 0.25, 0.44\}$,
$\sigma_m \in \{0.045, 0.075, 0.090, 0.135\}$, and
$\rho \in \{0.0, 0.3, 0.6, 0.9\}$, evaluating each of the $48$ cells over $100$
seeds.

\paragraph{Generation budget in the low-benefit regime.}
Finally, to test whether a low benefit merely slows the transition rather than
preventing it, we take the weakest regime ($B = 0.10$, low $\rho$, low
$\sigma_m$) and extend the evolutionary horizon to $240$, $480$, and $960$
generations over $100$ seeds, at two mutation scales.
