...
condition. In the fully vertical condition, communication remains weak within
this 30-generation horizon: only half of the seeds reach the
directional-precision threshold, and final payoff is close to the minimum floor.
This should be read as a short-run coordination problem rather than evidence
that gravity-referenced communication is unreachable.
...
strongly coupled. The next modeling priority is therefore to run longer
transition experiments from horizontal or tilted starts, using the axial
orientation representation, to test whether the apparent collapse is a true
failure mode or a short-run coordination lag. Only after that horizon is
established should the vertical-comb benefit and mutation scales be calibrated
to ask how architecture and communication can coevolve without a collapse in
foraging success.
