...
colonies begin with almost no communicative ability and forage by near-random
search, so foraging success is lowest in the founding generation and climbs as
the code evolves. The minimum $m_j$ is thus in effect the founders' foraging success,
and penalizing it steers the search away from settings in which foraging is never
viable. A seed's \emph{progress} measures how near it came to the
first two: each trait is expressed as a fraction of the threshold it must reach,
