...
codes without a breakdown of communication, and if so which. The search space is
large and costly to explore: eight parameters interact, and evaluating one
combination costs a full evolutionary run over a panel of seeds. We therefore
use optimization to give the transition its best chance of succeeding, treating
the search as a way of locating viable regions, and then probe the robustness of
what it finds in the sequence of sub-experiments below. Because a search that
maximizes over a seed panel will also fit that panel's noise, tuning and
evaluation draw from disjoint random seed blocks, with details specified below.
...
We search eight parameters with the Tree-structured Parzen Estimator sampler of Optuna \citep{akiba2019optuna}:
food-site count, patch radius, capacity, maximum distance, and travel cost,
together with $B$, $\sigma_m$, and $\rho$. \Cref{tab:search-space} gives the
interval and step size searched for each; the sampler proposes values on these
steps rather than continuously, so the search space is a discrete lattice rather
than a box. Colonies evolve for $120$ generations throughout. We run $1024$
trials per decode variant, distributed over sixteen compute nodes of thirty-two
workers each, all proposing against a single shared study so that every worker
samples in the light of every trial completed so far. Each worker draws from an
independently seeded sampler, and the sampler proposes at random until the study
has $64$ completed trials, after which it fits the estimator.
...
$\min(\bar t_{s,j}, \bar t_{r,j}) \ge t^\ast$, where $\gamma^\ast = 0.80$ and
$t^\ast = 0.50$, and \emph{collapsed} if $m_j \le m^\ast$ with $m^\ast = 0.02$.
Its \emph{progress} is how far the laggard of the three traits got towards its
threshold, capped at one,
\begin{equation}
  g_j = \min\Bigl(1,\; \frac{\bar\gamma_j}{\gamma^\ast},\;
    \frac{\bar t_{s,j}}{t^\ast},\; \frac{\bar t_{r,j}}{t^\ast}\Bigr),
\end{equation}
so that $g_j = 1$ holds exactly when seed $j$ is stable. The trial score is then
\begin{equation}
  \sum_{j=1}^{J} \mathbf{1}\bigl[g_j = 1\bigr]
  \;+\; \frac{1}{J}\sum_{j=1}^{J} g_j
  \;-\; \sum_{j=1}^{J} \mathbf{1}\bigl[m_j \le m^\ast\bigr].
\end{equation}
The stable-count term dominates while the progress term ranks near misses: a
whole extra stable seed is always worth more than any improvement in progress
across the panel.
