...
$d_{\max} = 6$ km, and foraging is charged $0.027$ per km travelled.
...
(\Cref{sec:vertical}), and we run the whole pipeline below under each. The
...
cost, together with $B$, $\sigma_m$, and $\rho$ (food value fixed at $v = 1$).
\Cref{tab:search-space} gives the interval and step size searched for each; the
sampler proposes values on these steps rather than continuously, so the search
space is a discrete lattice rather than a box. Colonies evolve for $120$
generations throughout. We
...
\begin{table}
  ...
  \begin{tabular}{lrr}
    ...
    \bottomrule
  \end{tabular}
  \caption{The space searched by the Optuna study. Each parameter is proposed on
