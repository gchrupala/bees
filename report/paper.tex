...
which are chosen by the experimenter and referred to by their mathematical
symbols. A colony is described by a small set of heritable, real-valued traits,
listed in full in \Cref{tab:traits}. The prose here introduces the traits needed
for the basic horizontal-comb model (\Cref{sec:horizontal}); the additional
traits and geometry required for tilted combs are developed in
\Cref{sec:vertical}.
...
\begin{table}
  ...
  \begin{tabular}{@{}cll p{0.42\textwidth}@{}}
    ...
    $b$      & \trait{directional bias}    & $[0,1]$ & sharpness of a scout's dance about the food direction \\
    $a$      & \trait{receiver attention}  & $[0,1]$ & probability of following a dance rather than searching at random \\
    $p$      & \trait{dance propensity}    & $[0,1]$ & readiness to recruit others after a successful forage \\
    $\ell$   & \trait{foray distance}      & $[0,D]$ & mean outbound distance of a foraging trip \\
    \addlinespace
    ...
    $\phi$   & \trait{comb orientation}       & $[0,P)$ & compass heading the comb faces (period $P$) \\
    $t_s$    & \trait{sender transposition}   & $[0,1]$ & weight on the gravity code when producing a dance \\
    $t_r$    & \trait{receiver transposition} & $[0,1]$ & weight on the gravity code when interpreting a dance \\
    \bottomrule
  \end{tabular}
...
call the direct code's strength and measure by the length of the
