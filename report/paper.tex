...
whether communication can survive as the comb tilts under that pressure. Because
...
surface discards part of it, and a follower must read a dance performed on that
tilted surface. How faithfully the heading is recovered depends on whether the
receiver corrects for the tilt, and we model two idealized decoders
(\Cref{sec:vertical}). The \emph{flatten} decoder applies no correction, reading
off the horizontal heading of the danced gesture; it is the more biologically
plausible of the two but leaves a systematic directional bias on any tilted comb.
The \emph{unproject} decoder instead inverts the projection exactly using the
comb's known tilt and orientation, removing the bias at the cost of assuming the
receiver has access to the comb's geometry; we keep it as a bias-free reference
against which to read the flatten results. At the same time the tilt makes the
gravity-referenced code available, so a colony can in principle switch codes as
direct pointing fails; whether it does is what these experiments test.
\subsubsection{Horizontal comb}
...
\subsubsection{Towards vertical combs}
\label{sec:vertical}
...
projects the food's world direction $\mathbf{f} = (\cos\alpha, \sin\alpha, 0)$, a
horizontal unit vector at azimuth $\alpha$, orthogonally onto the comb and dances
along the resulting in-surface vector $\mathbf{p}$ (\Cref{fig:direct-decode}).
...
projection has coordinates $p_i = \mathbf{e}_i\cdot\mathbf{f}$, and the encoded
signal is its angle $\delta_{\mathrm{dir}} = \operatorname{atan2}(p_2,p_1)$. The
code's \emph{strength} is how much of the unit direction survives the projection,
...
\end{equation}
On a horizontal comb $s_{\mathrm{dir}} = 1$: the direction lies entirely within
...
$\det M = \cos\theta$. Decoding recovers a world heading by inverting this map,
and we model two receivers, bracketing whether the geometric correction is done
in full (\Cref{fig:direct-decode}).
\begin{description}
  \item[Flatten] The receiver lifts the signal $\delta$ to the unit in-surface
    vector $\cos\delta\,\mathbf{e}_1 + \sin\delta\,\mathbf{e}_2$ and reads its
    horizontal heading $\mathbf{r}$, which applies $M^\top$ rather than
    $M^{-1}$. Because $M^\top \ne M^{-1}$ on a tilted comb, $\mathbf{r}$ is
    systematically biased there.
  \item[Unproject] The receiver instead applies $M^{-1}$, exactly inverting the
    encoding; since $\det M = \cos\theta > 0$ for $\theta < \pi/2$, the true
    heading is recovered without bias.
\end{description}
On a flat comb $M$ is a rotation, so $M^\top = M^{-1}$ and the two variants
coincide. The gravity code, by contrast, is always decoded by exact inversion:
...
be the in-surface angles the two codes assign to the food just visited. The
