...
between senders and receivers to avoid a breakdown of communication. The
honeybee waggle dance illustrates this problem: species with horizontal combs
point directly at a food source, while species with vertical combs cannot
point directly and instead reference the dance to gravity, decoded against the
position of the sun. We model the rise of the first of these codes and its evolutionary transition
to the second in populations of bee-like agents, with selection acting at the
level of colonies. In a horizontal-comb model, we find that
direct pointing evolves readily when food is moderately hard to find by random
search alone, whether because sites are few and large or many and small, and
fails when food is too sparse to spark dances or so abundant that it is found
without signaling. Adding an exogenous benefit for vertical combs, we then
find that the transition to the gravity-referenced code is driven mainly by
the mutation rate and the magnitude of this benefit, with the coupling between
sender and receiver mutations playing a further role at low mutation rates.
Given a favorable confluence of these factors, the transition proceeds
reliably and without a breakdown of communication.
\end{abstract}
...
seeds, because a search that maximizes over a panel of seeds will fit that
panel's noise along with its signal.
...
which foraging is never adequate. A seed's \emph{progress} measures how near it
...
The top five distinct candidates emerging from each search's $40$-seed
confirmation panel, rerun on $100$ held-out seeds
($200$--$299$), all produced frequent stable transitions and no non-viable seeds,
with flatten's candidates achieving somewhat higher stable rates than
unproject's. The strongest candidate of each decode (\Cref{tab:validation}) lands
...
long range, where flatten's tilt-dependent decoding bias makes precise dances
hard to produce, pushing its validated point to the edge of the search range on
...
useful, too much makes independent search sufficient on its own. 
...
benefit, and even then more readily under flatten than unproject, remaining
open everywhere else we tested. This is a claim about what is possible, not what was likely in the actual
evolutionary history of \emph{Apis}: that would need independent evidence on
ancestral foraging ecology, the real benefit of vertical combs, and the
genetic basis of dance production and interpretation. Feasibility is, however,
enough to answer the coordination problem posed in the introduction: a shared code can be revised
without a breakdown of communication across most of the conditions we
examined, not just at one finely tuned point among them.

\section{Limitations}
\label{sec:limitations}

Several aspects of the model bound what these results can support. Only the
dance's directional component is modeled, so the results say nothing about how
