...
% Sets off a model parameter or heritable trait name at its point of
% definition. Restyle here (e.g. \textsc) to change every occurrence at once.
...
evolutionary and reinforcement-learning dynamics. A distinct route
emphasizes cultural transmission: in the iterated-learning framework,
linguistic structure emerges as a code is repeatedly learned and reproduced
across successive generations of agents, without being built in
\citep{smith2003iterated}. Related formal work models
how a protolanguage itself evolves under selection, showing for instance how
combining a small set of signals into words overcomes the error limit that caps
a simple signal repertoire \citep{nowak1999evolution}. Agent-based models
in this tradition have aimed to demonstrate that populations playing repeated
language games can self-organize shared, grounded vocabularies and
categories, a simulation tradition surveyed by \citet{wagner2003progress}.
More recently, deep multi-agent
...
exists, \emph{when does it pay off}? The main tool here is the 
...
runs. The outcome of interest is colony-level foraging performance: typically the net
...
communication beats reliance on private information.  This body of
work is the closest in spirit to our own foraging payoff, but it holds the
...
tens of thousands of generations in a one-dimensional circular arena where a 
receiver must reach one of several sites a sender has perceived as holding food.
Their central finding concerns how a displaced signal
arises. Although senders could in principle modulate  signal
...
communication zone), which were subsequently ritualized into signals.
The more
efficient amplitude-based scheme evolved only when the cheaper cue was
experimentally suppressed. This model
...
transmit two real-valued tokens identifying the food node. They ask whether the two tokens factor into
...
Our study occupies a different point in this space (\Cref{tab:routes}). We model the emergence
and in particular the evolutionary \emph{transition} between two forms of a
referential code at the level of colonies, using a small set of interpretable,
heritable traits rather than opaque neural controllers. This lets us treat costs, benefits, 
and comb geometry as explicit
...
individual bee-like agents. Because a colony's workers are highly related and
largely sterile, selection acts effectively on heritable colony-level traits
...
\citep{hamilton1964genetical,seeley1989superorganism}. There is no sexual
reproduction: colonies reproduce without genetic input from other colonies. Colonies have a number of heritable
...
\label{sec:traits}
A colony is described by a small set of heritable, real-valued traits, listed in
full in \Cref{tab:traits}. The prose here introduces the traits needed for the
basic horizontal-comb model (\Cref{sec:horizontal}); the additional traits and
...
\begin{table}
  ...
  \begin{tabular}{@{}cll p{0.42\textwidth}@{}}
    ...
    $b$      & \param{directional bias}    & $[0,1]$ & sharpness of a scout's dance about the food direction \\
    $a$      & \param{receiver attention}  & $[0,1]$ & probability of following a dance rather than searching at random \\
    $p$      & \param{dance propensity}    & $[0,1]$ & readiness to recruit others after a successful forage \\
    $\ell$   & \param{foray distance}      & $[0,D]$ & mean outbound distance of a foraging trip \\
    \addlinespace
    \multicolumn{4}{@{}l}{\emph{Nest and code choice (vertical transition, \Cref{sec:vertical})}}\\
    $\gamma$ & \param{comb tilt}              & $[0,1]$ & comb inclination, $0$ horizontal to $1$ vertical \\
    $\phi$   & \param{comb orientation}       & $[0,P)$ & compass heading the comb faces (period $P$) \\
    $t_s$    & \param{sender transposition}   & $[0,1]$ & weight on the gravity code when producing a dance \\
    $t_r$    & \param{receiver transposition} & $[0,1]$ & weight on the gravity code when interpreting a dance \\
    \bottomrule
  \end{tabular}
  \caption{The heritable, colony-level traits of the model. The first group
    ...
    $t_r$ with individual variation (\Cref{sec:traits}); $\ell$ varies instead
...
Four traits govern the behavior of a colony's workers. The \param{directional
bias} $b$ controls how precisely a dance points; the \param{receiver attention}
$a$ is the propensity to attend to a dance rather than to search at random; the
\param{dance propensity} $p$ controls how readily a successful scout recruits
others to the patch it just found (\Cref{sec:foraging}); and the \param{foray
...
where $D$ is the \param{maximum search distance}, a fixed model parameter (set to
...
\end{equation}
where $\eta$ is a fixed \param{worker-variation scale} and $\mathrm{clip}$ returns the
...
as in \Cref{sec:traits}. The \param{mutation rate} $\sigma_m$ thus sets the pace of
evolutionary change.
...
colony-level traits describing the nest: the \param{comb tilt} $\gamma \in [0,1]$,
where $\gamma = 0$ is a horizontal comb and $\gamma = 1$ a vertical one, and the
\param{comb orientation} $\phi$, the compass heading the comb faces. Like the
...
coordinates $(p_1,p_2)^\top = M(\cos d,\sin d)^\top$, i.e.\ the in-surface vector
$\mathbf{p} = p_1\,\mathbf{e}_1 + p_2\,\mathbf{e}_2$, and the encoded signal is its
angle $\delta_{\mathrm{dir}} = \operatorname{atan2}(p_2,p_1)$; the
...
call the direct code's \param{strength} and measure by the length of the
...
the \param{sender transposition} $t_s$ and \param{receiver transposition} $t_r$
...
correlated Gaussian pair whose \param{sender--receiver coupling} is the
...
\begin{equation}
  1 + B\,\gamma,
\end{equation}
where $B \ge 0$ is the magnitude of the \param{vertical-comb benefit}, so that taller
...
\begin{figure}
  ...
  \caption{Evolved directional bias (left) and recruitment advantage (right) on a
