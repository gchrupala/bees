...
Each trial evaluates a panel of $J = 10$ seeds ($100$--$109$). A run's
\emph{foraging success} at a generation is the fraction of that generation's
foraging attempts that reach a food site (\Cref{sec:foraging}), averaged over the
colonies. For seed $j$ we write $\bar\gamma_j$, $\bar t_{s,j}$ and $\bar t_{r,j}$
for the final-generation population means of the \trait{comb tilt} and the two
\trait{transposition} traits, and $m_j$ for the lowest foraging success reached at
any generation of the run. A seed is \emph{stable} if $\bar\gamma_j \ge
...
the code evolves. The minimum $m_j$ is thus in effect the founders' success rate,
