...
within an episode but varies between episodes. Within an episode the sun is
shared by every worker, so when a dancer and follower both use the
gravity-referenced code the sun azimuth enters encoding and decoding with
opposite sign and cancels, and where it falls does not matter. Because it
varies across episodes, however, a colony cannot treat it as a fixed offset: a
strategy that does not fully commit to the gravity code leaves an uncancelled
sun term, so the shifting reference selects for genuine transposition.
\begin{figure}
  \centering
