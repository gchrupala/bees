...
fresh environment (\Cref{fig:environment}), with patch radii in meters and distances from the nest in kilometers.
...
is approximately $\arcsin(\mathrm{radius}/\mathrm{distance})$, with radius and
distance in the same units, and shrinks with distance: a fixed patch subtends a
smaller angle when farther from the nest. Each
...
fixed. In our experiments $v = 1$, the median radius is $\rho_r = 150$ m with
log-scale spread $\sigma_r = 0.6$, a patch at the median radius has capacity $6$,
and sites lie between $d_{\min} = 0.75$ km and $d_{\max} = 6$ km.
...
\begin{figure}
  \centering
    ...
    \caption{Six samples of the disk-geometry foraging environment. Each panel
      ...
      $6$ at that radius, drawn out to a maximum distance of $6$ km.}
...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    $D$ & $6$ km & maximum search distance \\
    $d_{\min}$ & $0.75$ km & minimum food distance \\
...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    $d_{\max}$            & $3$--$7.5$ km  & maximum patch distance \\
