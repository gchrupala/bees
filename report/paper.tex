...
categories \citep{steels2011modeling}. More recently, deep multi-agent
...
\todo{GC: A couple more of prominent works in this vein? Maybe one of Arianna Bisazza's paper?}
\todo{GC: Consider adding further anchors to this sketch: Nowak \&
...
does not model the code itself as an evolving trait.\todo{GC: clarify the reference to the benefit parameter.}

More recently, the emergence itself of the bee communicative code has been also been
...
receiver must reach one of several sites a sender has peeceived as hold food. 
...
\todo{GC: Reprhase kinamtic range in simpler language.}
...
\todo{GC:What's the main difference between these two latter works?}


Our study occupies a different point in this space. We model the emergence 
...
the underlying heading representation is indifferent to the spatial referencees.

frame, then the principal barrier to changing the code is not the neural machinery
but the ecological and coordination conditions that our model isolat
\section{Limitations}
