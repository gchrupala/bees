...
trip. The first three are bounded to $[0,1]$ and the \trait{foray distance} to $[0,D]$,
...
The \trait{foray distance} trait $\ell$ (\Cref{sec:traits}) sets how far a worker travels,
...
$\gamma$ is the \trait{comb tilt} and $B \ge 0$ the vertical-comb benefit
...
deviation $\sigma_m$ (scaled by $D$ for the \trait{foray distance}) and then clipped to
...
worker's \trait{transposition} with each channel's geometric strength,
\begin{equation}
