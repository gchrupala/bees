...
- `results/`: tracked result CSVs
- `report/`: `paper.tex`, its bibliography and figures
...
Regenerates all six generated figures from the tracked results (fast, no simulation),
...
The bibliography uses `apacite`, since *Artificial Life* follows APA style. On
Debian/Ubuntu TeX Live it comes from `texlive-bibtex-extra`; it can also be
installed into `$(kpsewhich -var-value TEXMFHOME)` from CTAN without root.
Each figure script writes both a PDF and a PNG: `paper.tex` omits the file
extension in `\includegraphics`, so the vector PDF is always the one typeset,
while the PNG stays available for slides and quick viewing.
...
The paper's simulations ran on the Snellius cluster (Slurm). `<user>` and
`<path-to-checkout>` below stand for your own account and checkout path. Sync your
...
```sh
./experiments/submit_food_transition_disk_pipeline_snellius.sh
```

This runs each decode's Optuna search as a Slurm array, then confirmation, validation,
sensitivity, and the interaction grid for both decodes. With `BEES_PUSH=1` (default `0`)
the final job also commits the merged result CSVs and pushes them to the checkout's
`origin`. Submit the horizontal-stage food-distribution grid separately:
...
The `.sbatch` scripts activate a project-local `.venv` by default; override with
`BEES_VENV` (venv path), `BEES_PYTHON` (interpreter), and `BEES_MODULE_LOAD` (module(s)
to load). Slurm on Snellius does not inherit the submitting shell's environment, so pass
any variable a *job* reads explicitly, e.g.
`sbatch --export=ALL,BEES_PUSH=1 job.sbatch`.

## License

The code, configs, and result data are released under the MIT License (see `LICENSE`).
The paper source and figures in `report/` are not covered by that license.
